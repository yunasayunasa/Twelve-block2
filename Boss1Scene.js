...
import {
    PADDLE_WIDTH_RATIO, PADDLE_HEIGHT, PADDLE_Y_OFFSET, BALL_RADIUS_RATIO, 
...
update(time, delta) {
   ...
   // ▲▲▲ 既にアクティブなら launch しない ▲▲▲
    ...
    setupPhysics() {
     ...
     // createPaddle メソッド (Y座標Clamp呼び出し追加)
     createPaddle() {
        console.log("Creating paddle in BossScene (with Y Clamp)..."); // ログ変更
        if (this.paddle) { this.paddle.destroy(); this.paddle = null; }
        const targetPaddleX = this.gameWidth / 2;
        const targetPaddleY = this.gameHeight - PADDLE_Y_OFFSET; // 目標Y座標

        this.paddle = this.physics.add.image(targetPaddleX, targetPaddleY, 'whitePixel')
            .setTint(0xffff00)
            .setImmovable(true)
            .setData('originalWidthRatio', PADDLE_WIDTH_RATIO);

        if (this.paddle) {
            this.updatePaddleSize();     // ★ サイズ初期設定 (X座標Clamp含む)
            this.clampPaddleYPosition(); // ★ Y座標Clamp処理を呼び出す
            console.log("Paddle created and position clamped in BossScene.");
        } else {
            console.error("!!! Failed to create paddle in BossScene !!!");
        }
    }
    ...
    createBalls() {
        ...
        if (this.paddle && this.paddle.active) {
            ...
            this.createAndAddBall(this.paddle.x, ballY);
             ...
             // パドルがない場合のフォールバックもClampしておく
             const fallbackTargetY = this.scale.height - PADDLE_Y_OFFSET;
             const fallbackPaddleHalfHeight = PADDLE_HEIGHT / 2;
             const fallbackMinY = this.scale.height - fallbackPaddleHalfHeight;
             const fallbackMaxY = this.scale.height / 2; // 例
             const fallbackClampedY = Phaser.Math.Clamp(fallbackTargetY, fallbackMaxY, fallbackMinY);
             this.createAndAddBall(this.scale.width / 2, fallbackClampedY - PADDLE_HEIGHT / 2 - BALL_RADIUS);
             console.warn("Paddle not available when creating ball, used fallback clamped Y.");
        }
        // ▲ ボール初期位置もClamp後のパドルY座標を考慮 ▲
...
testLogFunction(message) {
    ...
    // ▼▼▼ Y座標調整メソッド (新規追加 or 既存を修正) ▼▼▼
    clampPaddleYPosition() {
        if (!this.paddle) return;
        const paddleHalfHeight = this.paddle.displayHeight / 2;
        // 最低Y座標 (画面下端すれすれ)
        const minY = this.scale.height - paddleHalfHeight - 80; // 例: 30pxのマージン
        ...
        // 計算結果を Clamp する
        this.paddle.y = Phaser.Math.Clamp(targetY, maxY, minY); // Y座標は下が大きい値

        console.log(`[BossScene] Paddle Y clamped to: ${this.paddle.y.toFixed(0)} (Target: ${targetY.toFixed(0)}, MinY: ${minY.toFixed(0)}, MaxY: ${maxY.toFixed(0)})`);
        this.paddle.refreshBody(); // 念のためボディも更新
    }
    // ▲▲▲ Y座標調整メソッド ▲▲▲
