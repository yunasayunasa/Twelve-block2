...
hitAttackBrick(brick, ball) {
    ...
    // ★★★ speedMultiplier の宣言と初期化をここで行う ★★★
    let speedMultiplier = 1.0; // デフォルトは1.0
    if (ball.getData('isFast') && BALL_SPEED_MODIFIERS && POWERUP_TYPES) { // 定数の存在も確認
        speedMultiplier = BALL_SPEED_MODIFIERS[POWERUP_TYPES.SHATORA] || 1.0;
    } else if (ball.getData('isSlow') && BALL_SPEED_MODIFIERS && POWERUP_TYPES) {
        speedMultiplier = BALL_SPEED_MODIFIERS[POWERUP_TYPES.HAILA] || 1.0;
    }
    // ★★★--------------------------------------------------★★★

    // シーン固有の速度倍率（例: Boss4Sceneの試練IX）を取得するフック (任意)
    let sceneSpecificSpeedMultiplier = 1.0;
    if (typeof this.getSceneSpecificBallSpeedMultiplier === 'function') { // フックメソッドがあれば
        sceneSpecificSpeedMultiplier = this.getSceneSpecificBallSpeedMultiplier(ball);
    }
...
// (オプション) CommonBossScene に追加するフックメソッドの例
// getSceneSpecificBallSpeedMultiplier(ball) {
//     // デフォルトでは何も変更しない
//     return 1.0;
// }

// Boss4Scene.js でオーバーライドする例
// getSceneSpecificBallSpeedMultiplier(ball) {
//     if (this.isTrialShatoraActive && typeof this.currentTrialBallSpeedMultiplier === 'number') {
//         return this.currentTrialBallSpeedMultiplier;
//     }
//     return 1.0;
// }
