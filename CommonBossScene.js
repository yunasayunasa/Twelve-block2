...
export default class CommonBossScene extends Phaser.Scene {
    ...
    handleResize(gameSize){this.gameWidth=gameSize.width;this.gameHeight=gameSize.height;this.calculateDynamicMargins();this.updatePaddleSize();this.clampPaddleYPosition();if(this.boss?.active)this.updateBossSize(this.boss,this.bossData.textureKey,this.bossData.widthRatio);this.balls?.getChildren().forEach(b=>{const nBR=this.gameWidth*BALL_RADIUS_RATIO;b.setDisplaySize(nBR*2,nBR*2);if(b.body)b.setCircle(nBR*0.9);});this.powerUps?.getChildren().forEach(i=>i.setDisplaySize(this.gameWidth*POWERUP_SIZE_RATIO,this.gameWidth*POWERUP_SIZE_RATIO));if(this.scene.isActive('UIScene'))this.events.emit('gameResize');this.gameOverText?.setPosition(this.gameWidth/2,this.gameHeight/2).setFontSize(`${this.calculateDynamicFontSize(40)}px`);this.gameClearText?.setPosition(this.gameWidth/2,this.gameHeight*0.4).setFontSize(`${this.calculateDynamicFontSize(48)}px`).setWordWrapWidth(this.gameWidth*0.9);}
