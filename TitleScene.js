...
export default class TitleScene extends Phaser.Scene {
     constructor() {
        ...
        this.selectedBossStartValue = 1; // 初期選択ボス
        ...
        this.difficultySettings = {
            ...
            shura:    { name: "おに",     initialLives: 1,  maxLives: 1  },
        };
        ...
        this.selectedBossButtonTextObject = null; // ボタンのテキストオブジェクト用
     }
    ...
    create() {
        ...
        // --- 1. 背景 ---
        this.add.image(w / 2, h / 2, 'gameBackground3').setOrigin(0.5, 0.5).setDisplaySize(w, h);
        ...
        this.add.text(w / 2, h * 0.15, 'はちゃめちゃ！\n十二神将会議2！\n〜ぼすらっしゅ！〜', {
            fontSize: `${Math.min(h * 0.06, w / 8)}px`,
        ...
        sliderContainer.style.width = 'clamp(300px, 80%, 500px)';
        ...
        rateValueSpan.textContent = this.selectedRate.toString() + '%'; rateValueSpan.style.color = '#FFF'; rateValueSpan.style.fontSize = '16px'; rateValueSpan.style.minWidth = '3.5em'; rateValueSpan.style.textAlign = 'right'; rateValueSpan.style.marginRight = '10px';
        ...
        // --- 5. 難易度選択ドロップダウンリスト ---
        const difficultyDropdownY = sliderY + 80; // スライダーの下 (固定値で調整)
        ...
        difficultySelectDropdown.addEventListener('change', (event) => {
            ...
            this.updateSelectedBossButtonText();
        });
        ...
        // --- 7. 「選択したボスから挑戦」ボタン ---
        const fightSelectedBossButtonY = bossDropdownY + 65;
        ...
        this.selectedBossButtonTextObject = this.add.text(w / 2, fightSelectedBossButtonY, "", fightSelectedBossButtonStyle)
            ...
            .on('pointerdown', () => {
                ...
                console.log(`"挑戦する" button clicked. Starting ${targetSceneKey} with difficulty ${this.selectedDifficultyKey}`, testData);
                this.sound.play(AUDIO_KEYS.SE_START); this.stopTitleBgm(); this.clearDOM();
                try {
                     if (this.scene.manager.keys[targetSceneKey]) { this.scene.start(targetSceneKey, testData); }
        ...
        const startRushButtonY = fightSelectedBossButtonY + 85;
        ...
        startButtonBg.fillStyle(0x388E3C, 0.9).fillRoundedRect(-startButtonW / 2, -startButtonH / 2, startButtonW, startButtonH, startButtonRadius);
