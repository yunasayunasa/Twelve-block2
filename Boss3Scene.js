...
spawnWallBlock(line) { // line は 'A' または 'B'
    ...
    if (line === 'A') { // 右から左へ、上層 (ボスにめり込む)
        ...
        velocityX = -speed;
        spawnY = bossDisplayTopY + (this.gameHeight * (this.bossData.wallLineAYOffsetRatio || 1.0));
    } else { // line === 'B', 左から右へ、下層
        spawnX = -(this.gameWidth * scale * 0.5) - 10; // 画面左外
        velocityX = speed;
        spawnY = bossDisplayTopY + (this.gameHeight * (this.bossData.wallLineBYOffsetRatio || 1.1));
    }
