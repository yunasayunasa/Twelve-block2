...
export default class GameScene extends Phaser.Scene {
    ...
    create() {
        ...
        // --- ▼ パドル作成 & Y座標 Clamp ▼ ---
        console.log("Creating paddle in GameScene (with Y Clamp)...");
        const targetPaddleX = this.scale.width / 2; // Xは中央
        const targetPaddleY = this.scale.height - PADDLE_Y_OFFSET; // 目標Y座標
        ...
        if (this.paddle) {
            ...
            console.log("Paddle created and position clamped in GameScene.");
        } else {
            console.error("!!! Failed to create paddle in GameScene !!!");
        }
        // --- ▲ パドル作成 & Y座標 Clamp ▲ ---
        ...
        if (this.paddle && this.paddle.active) {
            ...
            // パドルがない場合のフォールバックY座標もClampしておくのが安全かも
            const fallbackTargetY = this.scale.height - PADDLE_Y_OFFSET;
            const fallbackPaddleHalfHeight = PADDLE_HEIGHT / 2; // 定数から計算
    ...
    clampPaddleYPosition() {
        ...
        console.log(`[GameScene] Paddle Y clamped to: ${this.paddle.y.toFixed(0)} (Target: ${targetY.toFixed(0)}, MinY: ${minY.toFixed(0)}, MaxY: ${maxY.toFixed(0)})`);
